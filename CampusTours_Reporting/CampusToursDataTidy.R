...
FM <- FM %>%
  filter(Code_Audience == "HS" & Code_Objective %in% objective) %>% 
  mutate(
...
write.csv(NewData, "C:/Users/christine.iyer/Box/quarterly-ad-analytics-projects/FlowchartCampusTours.csv", row.names = F)
